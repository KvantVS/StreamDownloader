...
def OneHourStop():
    ...
    # d.total_seconds()
    while 1:
        print('OneHourStop loop iterate...')
        if (datetime.datetime.now() - time_start).total_seconds() > 60:
            print('total_seconds() > 60. Exiting.')
            #sys.exit()
            #current_thread().interrupt_main()         
            _thread.interrupt_main()
            print('active_count:', threading.active_count())
            print('current_thread:', threading.current_thread())
            print('enum:', threading.enumerate())
            print('Main:', threading.main_thread())   
            sys.exit(0)
            print('active_count:', threading.active_count())
            print('current_thread:', threading.current_thread())
            print('enum:', threading.enumerate())
...
def mainThread():
    global client_id, non_bmp_map, debug, curdir
    ...
    if len(sys.argv) > 1:
        ...
        arg = 'https://www.twitch.tv/disguisedtoast'  # 'https://www.twitch.tv/artgameslp'  # sholidays #blackufa #artgameslp
...
th_main = Thread(target=mainThread, args=())
th_onehour = Thread(target=OneHourStop, args=(), daemon=True)
th_onehour.start()
th_main.start()
th_onehour.join()
th_main.join()
